...
## 11. HPS CLI 自身维护与运行规范 (Meta-Rules)

### 11.1 进程与终端交互
- **[强制] 外部命令执行**：在 `hps init` 或任何需要执行外部交互式命令（如 `pnpm create`, `npm install`）的场景，必须使用 `stdio: 'inherit'`。
- **[原因]**：`stdio: 'pipe'` 会拦截所有输出和输入，导致交互式工具（如 pnpm 的安装确认、权限提示）在后台静默挂死，用户无法操作。

### 11.2 UI 渲染器保护
- **[强制] 进度条管理**：在执行会产生大量终端输出的命令前，必须显式调用 `log.suspend()`。
- **[动作]**：`log.suspend()` 会重置渲染器的行数跟踪并清空当前进度条行。执行完命令后，应打印一个换行 `\n` 以平滑过渡回进度条模式。

### 11.3 知识库分发
- **[强制] 全量同步**：在 `hps init` 流程中，必须将 `resources/docs_summaries` 下的所有 `.md` 文档物理拷贝到项目本地的 `.hps/knowledge/` 目录。
- **[目标]**：确保在非 CLI 环境（如 Gemini Web, AI Studio）中，AI 能直接通过读取项目文件获得全量 Halo 技术细节。
